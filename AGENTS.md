...
2. **`ailayer/`** — a Python CLI that consumes those READMEs as "skills" and injects them (plus hooks, MCP servers, and instruction blocks) into agentic coding tools (Codex, Codex CLI, Gemini CLI).
...
ailayer add skill <name> --tool {Codex|codex|gemini|all} [--no-global --project DIR]
...
|---|---|---|---|---|
| `ClaudeCodeAdapter` | `Codex` | `~/.Codex/AGENTS.md` or `<proj>/AGENTS.md` | `~/.Codex/settings.json` (JSON) | `~/.Codex/commands/<name>.md` |
...
- Codex: `PreToolUse`, `PostToolUse`, `Stop`, `Notification`
...
- **JSON/YAML entries** (Codex/Gemini hooks): each injected hook entry carries a `_ailayer_name` field. `add_hook` checks for that name before inserting; `remove_hook` filters by it. MCP servers are keyed by their own name field, so no extra tag is needed there.
